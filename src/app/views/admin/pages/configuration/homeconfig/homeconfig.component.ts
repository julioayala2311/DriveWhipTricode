...
import { HomeGridComponent } from '../../home/home-grid.component';
...
import { HomeDialogComponent, LocationDialogResult } from '../../home/home-dialog.component';
...
@Component({
  ...
  styleUrl: './homeconfig.component.scss'
})
export class HomeConfigComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  locationsList(): void {
    ...
    this.driveWhipCore
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('[HomeConfigComponent] list error', err);
          this.locationsRows = [];
          this.rowsForGrid = [];
  ...
  openEdit(rec: LocationsRecord): void {
    ...
    this._editing.set(rec);      // â¬…ï¸ pasa el registro al dialog
    this._dialogOpen.set(true);  // â¬…ï¸ abre
  }
  ...
  /** CRUD â†’ ajusta el orden si tu SP difiere
  ...
  private mutate(action: 'C'|'U'|'D', rec: Partial<LocationsRecord> & {
    ...
    this.driveWhipCore
      ...
      .subscribe({
        next: (res) => {
          if (!res.ok) {
            console.error('[HomeConfigComponent] mutate error', res.error);
        ...
        error: (err) => {
          console.error('[HomeConfigComponent] mutate error', err);
          this._saving.set(false);
          this._loading.set(false);
        }
      });
