...
import {
  ApplicantActionsCellComponent,
  ApplicantQuickAction,
} from "./applicant-actions-cell.component";
...
@Component({
  ...
  template: `
    <div
      class="d-inline-flex align-items-center gap-1 header-cell"
...
@Component({
  ...
  template: `
    <div class="d-flex flex-wrap gap-2 align-items-center mb-4">
      <h6 class="mb-0 fw-semibold">
        Applicants
        <span *ngIf="applicantsCount !== null && applicantsCount !== undefined"
          >({{ applicantsCount }})</span
        >
        <span
          *ngIf="
    ...
    <div *ngIf="error" class="alert alert-danger py-1 px-2 small mb-2">
      {{ error }}
    </div>
    <div
      *ngIf="selectedCount > 0"
      class="badge bg-primary-subtle text-primary fw-normal mb-2 selection-badge"
    >
    ...
    <app-applicant-panel
      *ngIf="panelOpen"
      ...
      [currentStageId]="
        activeApplicant?.stageId ?? selectedStage?.id_stage ?? null
      "
      [uploadFiles]="activeApplicant?.uploadFiles ?? false"
  ...
  styles: [
    `
      ...
      :host ::ng-deep .ag-theme-quartz .ag-cell-wrap-text {
        white-space: normal !important;
      }
      :host ::ng-deep .ag-theme-quartz .status-badge {
        line-height: 1;
      }
      :host ::ng-deep .ag-theme-quartz .status-badge-action {
        cursor: pointer;
      }
      :host ::ng-deep .ag-theme-quartz .ag-cell div.d-flex.flex-wrap {
        align-items: center;
        gap: 0.25rem;
      }
      :host ::ng-deep .ag-theme-quartz .actions-cell-wrapper {
        padding: 0 !important;
      }
    `,
  ],
...
export class ApplicantsGridComponent implements OnInit, OnChanges {
  ...
  columnDefs: ColDef[] = [
    // { headerName: '', checkboxSelection: true, headerCheckboxSelection: true, width: 48, pinned: 'left', sortable: false, filter: false, resizable: false, suppressSizeToFit: true },
    {
      ...
      cellClass: "actions-cell-wrapper",
      cellRenderer: "applicantActionsCell",
    },
    {
      headerName: "Name",
      field: "name",
    ...
    },
    {
      ...
      flex: 0.8,
      headerComponent: GridHeaderComponent,
      headerComponentParams: { icon: "icon-phone" },
    },
    {
      headerName: "Status",
      field: "status",
      ...
      cellRenderer: (p: any) => {
        ...
        const renderOne = (item: any) => {
          ...
          const actionAttr = isReviewFiles
            ? ' data-status-action="open-files"'
            : "";
        ...
        for (let i = list.length - 1; i >= 0; i--) {
          const it = list[i];
          const key = `${(it.stage || "").toLowerCase()}|${(
            it.statusName || ""
          ).toLowerCase()}|${it.order ?? ""}`;
          if (!seen.has(key)) {
            seen.add(key);
            deduped.unshift(it);
          }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      changes["selectedWorkflowId"] &&
      !changes["selectedWorkflowId"].firstChange
    ) {
      if (this.cardId) this.loadApplicants();
    }
      ...
      next: (res) => {
        ...
        const mapped = list.map((r: any) => {
          ...
          const applicantId =
            ...
            r?.idApplicants ??
            null;
          // UploadFiles flag from SP to control file action availability
          const uploadFilesRaw =
            ...
            null;
          const uploadFiles =
            typeof uploadFilesRaw === "string"
              ? /^(true|1|y|yes)$/i.test(uploadFilesRaw)
              : !!uploadFilesRaw;
          return {
        ...
        if (this.activeApplicant) {
          const refreshed = mapped.find((m) =>
            this.isSameApplicant(m, this.activeApplicant as ApplicantRow)
          );
          if (refreshed) {
  ...
  private parseStatuses(rawStatusJson: any): {
    ...
    const list = arr
      ...
      .map((x: any) => {
        ...
        const statusNameRaw = String(x.statusName || x.status || "")
          .trim()
          .toLowerCase();
        const normalized =
          statusNameRaw === "complete" ? "complete" : "incomplete";
  ...
  private refreshGrid() {
    if (this.gridApi) {
      ...
      // Re-apply any pending quick filter after data refresh
      if (this._quickFilterText !== null) {
        try {
          (this.gridApi as any).setGridOption?.(
            "quickFilterText",
            this._quickFilterText
          );
        } catch {
          /* ignore */
        }
  ...
  onCellClicked(event: CellClickedEvent): void {
    ...
    if (event.colDef.field === "status" && event.data && clickTarget) {
      const actionable = clickTarget.closest(
        "[data-status-action]"
      ) as HTMLElement | null;
  ...
  handleQuickAction(
    ...
    if (!applicantId) {
      Utilities.showToast(
        "No pudimos identificar a este solicitante. Intenta actualizar la lista.",
        "warning"
      );
      return;
    }
    ...
    switch (action) {
      ...
      case "editApplicant":
        this.ensurePanelAction(applicantId, (panel) =>
          panel.startEditApplicant()
        );
        break;
      case "deleteApplicant":
        this.ensurePanelAction(applicantId, (panel) =>
          panel.onDeleteApplicantClick()
        );
        break;
      default:
        break;
  ...
  private ensurePanelAction(
    applicantId: string,
    action: (panel: ApplicantPanelComponent) => void,
    tab: PanelTab = "messages"
  ): void {
    if (!applicantId) return;

    const sameApplicant =
      this.panelOpen && this.activeApplicant?.id === applicantId;
  ...
  private deferPanelAction(
    ...
    setTimeout(
      () => this.deferPanelAction(action, attempt + 1, expectedId),
      50
    );
  }
  ...
  private enrichApplicantMeta(applicant: ApplicantRow): void {
    ...
    applicant.stageIcon =
      this.stageIconClass || applicant.stageIcon || "icon-layers";
  }

  private applyLocationToRows(): void {
    const loc = (this.locationName ?? "").trim();
    this.rowData.forEach(
      (row) => (row.locationName = loc || row.locationName || "")
    );
    this.recentApplicants.forEach(
      (row) => (row.locationName = loc || row.locationName || "")
    );
    this.applyPanelMeta();
  }
  ...
  private buildDetails(raw: any): { label: string; value: string }[] {
    ...
    return details.length
      ? details
      : [{ label: "Details", value: "No additional information available." }];
  }
  ...
  onApplicantDeleted(applicantId: string): void {
    ...
    this.recentApplicants = this.recentApplicants.filter(
  ...
  onFirstDataRendered() {
    ...
    this.updatePaginationState();
    this.gridApi?.resetRowHeights();
    // Apply quick filter if provided before grid was ready
    if (this.gridApi && this._quickFilterText !== null) {
      try {
        (this.gridApi as any).setGridOption?.(
          "quickFilterText",
          this._quickFilterText
  ...
  private updatePaginationState() {
    ...
    const pageSize =
      (this.gridApi as any).getGridOption?.("paginationPageSize") ||
      this.pageSize;
  ...
  // --- Quick filter support (for navbar search integration) ---
  private _quickFilterText: string | null = null;
  public setQuickFilter(text: string | null | undefined): void {
    const val = text == null ? null : String(text);
    this._quickFilterText = val;
    if (this.gridApi) {
      try {
        (this.gridApi as any).setGridOption?.("quickFilterText", val || "");
      } catch {
        /* ignore */
      }
    }
